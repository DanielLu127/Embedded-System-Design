...
int MT_PUT_construct_Helper(int mtAction, int mtDegree, char buffer[], int *seqNumber) {
    ...
    (*seqNumber)++;
    sprintf(buffer, "PUT /motor4/%d HTTP/1.1\r\nHost: 192.168.0.10:80\r\nContent-type: application/json\r\nContent-length:%d \r\n\r\n", *seqNumber, contentLength);
    strcat(buffer, temp);
    vPortFree(temp);
...
int SONAR_PUT_construct_Helper(int sonarDir, int sonarDis, char buffer[], int *seqNumber) {
    ...
    int contentLength = strlen(temp);
    (*seqNumber)++;
    sprintf(buffer, "PUT /sonar4/%d HTTP/1.1\r\nHost: 192.168.0.10:80\r\nContent-type: application/json\r\nContent-length:%d \r\n\r\n", *seqNumber, contentLength);
    strcat(buffer, temp);
    vPortFree(temp);
    int totalSize = strlen(buffer);
    return totalSize;
...
{
    cJSON * rootObject = cJSON_CreateObject();
    cJSON_AddItemToObject(rootObject,"name", cJSON_CreateString("B4"));
    ...
    int contentLength = strlen(temp);
    (*seqNumber)++;
    sprintf(buffer, "PUT /stat3/%d HTTP/1.1\r\nHost: 192.168.0.10:80\r\nContent-type: application/json\r\nContent-length:%d \r\n\r\n", *seqNumber, contentLength);
    strcat(buffer, temp);
    vPortFree(temp);
    int totalSize = strlen(buffer);
    return totalSize;
...
int PIXYMOTOR_PUT_construct_Helper(Pixy2Obj data,  char buffer[], int * seqNumber){
    ...
    int contentLength = strlen(temp);
    (*seqNumber)++;
    sprintf(buffer, "PUT /pixy4/%d HTTP/1.1\r\nHost: 192.168.0.10:80\r\nContent-type: application/json\r\nContent-length:%d \r\n\r\n", *seqNumber, contentLength);
    strcat(buffer, temp);
    vPortFree(temp);
    int totalSize = strlen(buffer);
    return totalSize;
