...
void pixy2fsm(PIXY2_THREAD_MSG  * curState)
{
    ...
    switch(temp.Type){
        ...
        case RECEIVE:
            ...
            if( (byte_counter == (num_vars*14 + 6) )  || (num_vars == 0  && byte_counter == 6) || (blocked_count ==2 ) )    { // this means our pixy_thread queue is empty, if the last message was of type init and there's no back log from the receive buffer, more than likely it's empty
