...
{
    BaseType_t conSwitch = pdFALSE;
    sensorMessageType test1;
    
    if(SYS_INT_SourceStatusGet(INT_SOURCE_USART_4_RECEIVE)){
        test1.inputData = PLIB_USART_ReceiverByteReceive (USART_ID_4);
        test1.value = 0;
        sendFromISRtoQueue(test1, &conSwitch);
        SYS_INT_SourceStatusClear(INT_SOURCE_USART_4_RECEIVE);
    }
    
    if(SYS_INT_SourceStatusGet(INT_SOURCE_USART_4_TRANSMIT)){
        PLIB_USART_TransmitterByteSend(USART_ID_4, test1.inputData);
