...
def goStraight():
    ...
    collection.update_one({"name":"motor4"}, {"$set":
        {"mtAction":2,"mtDegree":0}})
    while True:
        motor = collection.find_one({"name": "motor4"})
        if motor['mtAction'] == 2:
           time.sleep(0.1)
    ...
    collection.update_one({"name":"motor4"}, {"$set": {"mtAction":4,"mtDegree":0}})
    print ("Finish action and stop motor")

def goBack():
    print ("Send action command: Go Back")
    collection.update_one({"name":"motor4"}, {"$set": {"mtAction":3,"mtDegree":0}})
    input("Press Enter to continue...")
    collection.update_one({"name":"motor4"}, {"$set": {"mtAction":4,"mtDegree":0}})
    print ("Finish action and stop motor")
...
def turnRight():
    ...
    collection.update_one({"name":"motor4"}, {"$set": {"mtAction":5,"mtDegree":0}})
    while True:
        motor = collection.find_one({"name": "motor4"})
        if motor['mtAction'] == 1:
            break
...
def turnRightSpec(num):
    ...
    collection.update_one({"name":"motor4"}, {"$set": {"mtAction":8,"mtDegree":num}})
    while True:
        motor = collection.find_one({"name": "motor4"})
        if motor['mtAction'] == 1:
            break
...
def turnLeft():
    ...
    collection.update_one({"name":"motor4"}, {"$set": {"mtAction":6,"mtDegree":0}})
    while True:
        motor = collection.find_one({"name": "motor4"})
        if motor['mtAction'] == 1:
            break
...
def turnLeftSpec(num):
    ...
    collection.update_one({"name":"motor4"}, {"$set": {"mtAction":7,"mtDegree":num}})
    while True:
        motor = collection.find_one({"name": "motor4"})
        if motor['mtAction'] == 1:
            break
