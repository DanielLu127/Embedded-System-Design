...
void send_2_Tx_queue(Tx_msg msg){
 ...
 void game_Send_Data(int board, int mtAction, int mtDegree, int armAction) {
     WiFiQueType msg = {.type = TX, .req = PUT, .name = GAME_NAME, 
                        .board = board, .mtAction = mtAction, 
                        .mtDegree = mtDegree, .armAction = armAction};
     send_2_WIFI_QUEUE(msg); 
 }
 
  void game_send_GET() {
      WiFiQueType msg = {.type = TX, .req = GET, .name = GAME_NAME};
      send_2_WIFI_QUEUE(msg); 
  }